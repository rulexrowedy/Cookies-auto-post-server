...
def _find_chromedriver_path():
    """Return chromedriver binary path, trying several methods."""
    ...
    # 2. which command
    try:
        r = subprocess.run(['which', 'chromedriver'],
...
def setup_browser(session):
    ...
    if chrome_bin:
        ...
        session.log(f'Using browser: {chrome_bin}')

    drv_path = _find_chromedriver_path()
    if drv_path:
        session.log(f'Using chromedriver: {drv_path}')
        svc    = Service(executable_path=drv_path)
        driver = webdriver.Chrome(service=svc, options=opts)
    else:
        session.log('Chromedriver not found at known paths, using selenium-manager...')
...
def open_profile_composer(driver, session, fb_uid):
    """Navigate to logged-in user's own profile and open the post composer."""
    profile_url = f'https://www.facebook.com/{fb_uid}'
    session.log(f'Opening profile: {profile_url}')
    ...
    """)
    if opened:
        session.log('Composer opened via JS click.')
        time.sleep(4)
    else:
        session.log('Composer button not found by text, trying aria-label selectors...')
        for sel in [
    ...
    # Find textbox inside composer dialog
    for sel in [
    ...
    session.log('Composer textbox NOT found.')
    return None


def attach_photo(driver, session, img_abs_path):
    """Find the file input inside the composer dialog and send the image path."""
        ...
        for sel in [
            ...
            if btns:
                try:
                    ...
                    pass
        session.log('No file input found — skipping image.')
        return False
    except Exception as e:
        session.log(f'Image attach error: {str(e)[:50]}')
        return False


def click_post_button(driver, session):
    """Click the Post / Share Now / Publish button in the composer."""
    session.log('Submitting post...')
    clicked = driver.execute_script("""
        var candidates = document.querySelectorAll('[role="button"]');
    ...
    # Fallback: aria-label="Post"
    for sel in ['div[aria-label="Post"][role="button"]',
                'button[type="submit"]']:
        elems = driver.find_elements(By.CSS_SELECTOR, sel)
        if elems:
            try:
...
def run_post_session(session, cookies, text_lines, prefix,
    ...
    while session.running and retries < 10:
        try:
            ...
            # Resolve mention UIDs → names (done once per session start)
            resolved_mentions = mention_names  # already resolved before thread start
            ...
            while session.running:
                try:
                    ...
                    if resolved_mentions:
                        parts.append(' '.join(resolved_mentions))
                    if line.strip():
                        parts.append(line.strip())
                    post_text = ' '.join(parts)

                    session.log(f'Composing post #{session.count+1}: {post_text[:50]}...')

                    textbox = open_profile_composer(driver, session, fb_uid)
                    if not textbox:
                        fail_count += 1
                        session.log(f'Composer not found ({fail_count}/5)')
                        if fail_count >= 5:
                            session.log('Too many failures — stopping.')
                            session.running = False
                            break
                        driver.refresh()
                        time.sleep(15)
                    ...
                    if image_paths:
                        ...
                        session.img_idx += 1
                        session.log(f'Image {session.img_idx}: {Path(img).name}')
                        attach_photo(driver, session, img)
                        time.sleep(2)
                    ...
                    if ok:
                        ...
                        session.log('Post may have failed, closing composer...')
                        try:
                            driver.execute_script("""
                                var cls = document.querySelector('[aria-label="Close"]');
                                if(cls) cls.click();
                            """)
                        except: pass
                        time.sleep(5)
